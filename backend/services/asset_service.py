...
from backend.models import Asset, MaintenanceLog, User
...
class AssetService:
    ...
    @staticmethod
    def create_asset(user_id: int, asset_data: Dict) -> Asset:
        """
        ...
        try:
            ...
            asset_id = f"AST-{datetime.utcnow().strftime('%Y%m%d')}-{user_id}-{Asset.query.count() + 1:04d}"
    ...
    @staticmethod
    def update_telemetry(asset_id: str, telemetry_data: Dict) -> Tuple[Asset, float]:
        """
    ...
    @staticmethod
    def _calculate_health_score(asset: Asset, telemetry: Dict) -> float:
        """
    ...
    @staticmethod
    def _check_maintenance_due(asset: Asset):
    ...
    @staticmethod
    def _build_prediction_prompt(asset: Asset, maintenance_logs: List[MaintenanceLog]) -> str:
...
"""
    ...
    @staticmethod
    def _parse_ai_prediction(ai_response: str, asset: Asset) -> Dict:
    ...
    @staticmethod
    def _fallback_prediction(asset: Asset) -> Dict:
    ...
    @staticmethod
    def get_assets_by_user(user_id: int, filters: Optional[Dict] = None) -> List[Asset]:
        """
        ...
        if filters:
            ...
            if 'health_min' in filters:
                query = query.filter(Asset.health_score >= filters['health_min'])
            
            if 'health_max' in filters:
                query = query.filter(Asset.health_score <= filters['health_max'])
        
        return query.order_by(Asset.created_at.desc()).all()
    
    @staticmethod
    def get_critical_assets(health_threshold: float = HEALTH_CRITICAL) -> List[Asset]:
        """Get all assets with health below threshold."""
        return Asset.query.filter(
            and_(
                Asset.health_score < health_threshold,
                Asset.status == 'ACTIVE'
            )
        ).order_by(Asset.health_score).all()
